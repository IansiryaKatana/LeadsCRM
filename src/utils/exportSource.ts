...
// Fetch leads filtered by date range and source
...
export async function exportSourceToExcel(data: SourceExportData) {
  const { startDate, endDate, currencySymbol, sourceSlug, sourceName } = data;
  
  // Fetch leads for the date range and source
  ...
  // Use the existing export function with source name in title
