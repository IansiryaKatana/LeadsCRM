  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SourceReportsData> => {
      ...
      let query = supabase
        .from("leads")
        .select(
          "created_at, lead_status, potential_revenue, room_choice, phone, academic_year, source",
        )
      ...
      const converted = rows.filter((l) => l.lead_status === "converted").length;
      const conversionRate = totalLeads > 0 ? (converted / totalLeads) * 100 : 0;
      const totalRevenue = rows
        .filter((l) => l.lead_status === "converted")
        .reduce((sum, l) => sum + (l.potential_revenue || 0), 0);
      const forecastRevenue = rows
        .filter((l) => l.lead_status === "high_interest")
        .reduce((sum, l) => sum + (l.potential_revenue || 0), 0);

      const stats: DashboardStats = {
        totalLeads,
        newLeads: rows.filter((l) => l.lead_status === "new").length,
        awaitingOutreach: rows.filter((l) => l.lead_status === "awaiting_outreach").length,
        lowEngagement: rows.filter((l) => l.lead_status === "low_engagement").length,
        highInterest: rows.filter((l) => l.lead_status === "high_interest").length,
        converted,
        closed: rows.filter((l) => l.lead_status === "closed").length,
        conversionRate,
        totalRevenue,
        forecastRevenue,
      };
      ...
      rows.forEach((lead) => {
        ...
        if (monthlyMap.has(key)) {
          ...
          if (lead.lead_status === "converted") {
            bucket.converted += 1;
            bucket.revenue += Number(lead.potential_revenue) || 0;
          }
        }
      });
